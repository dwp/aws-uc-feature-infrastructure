...
(
    ...
    function log_wrapper_message() {
        log_aws_uc_feature_message "$${1}" "emr-setup.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
...
EOF
    ...
    --jks-only true >> /var/log/aws_uc_feature/acm-cert-retriever.log 2>&1
    ...
    --private-key-alias "${private_key_alias}" \
    --truststore-aliases "${truststore_aliases}" \
    --truststore-certs "${truststore_certs}"  >> /var/log/aws_uc_feature/acm-cert-retriever.log 2>&1 # No sudo needed to write to file, so redirect is fine
...
) >> /var/log/aws_uc_feature/emr-setup.log 2>&1
