...
sudo mkdir -p /var/log/aws_uc_feature_infrastructure
sudo mkdir -p /opt/emr
sudo mkdir -p /opt/shared
sudo mkdir -p /var/ci
sudo chown hadoop:hadoop /var/log/aws_uc_feature_infrastructure
...
echo "${aws_uc_feature_infrastructure_LOG_LEVEL}" > /opt/emr/log_level
...
(
    ...
    function log_wrapper_message() {
        log_aws_uc_feature_infrastructure_message "$${1}" "download_scripts.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
...
)  >> /var/log/aws_uc_feature_infrastructure/download_scripts.log 2>&1
